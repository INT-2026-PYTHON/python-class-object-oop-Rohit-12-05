...
class Student:
    """A class representing a student with a list of subject marks."""

    def __init__(self, name, roll, marks=None):
        self.name = name
        self.roll = roll
        
        # Safely handle the mutable default argument
        if marks is None:
            self.marks = []
        else:
            self.marks = marks
    ...
    def add_mark(self, mark):
        ...
        if mark < 0 or mark > 100:
            print(f"Invalid mark rejected for {self.name}: {mark}. Must be 0-100.")
        else:
    ...
    def average(self):
        # Prevent DivisionByZero error if the student has no marks yet
        if len(self.marks) == 0:
            return 0
        return self.total() / len(self.marks)

    def grade(self):
        # Calling an instance method from another instance method using 'self'
        avg = self.average()
    ...
    def report(self):
        # Return a tuple summarizing the student's data
...
# Test an invalid mark rejection
s2.add_mark(-10)
s2.add_mark(105)

# 3. Print the report tuples
print(s1.report())
print(s2.report())